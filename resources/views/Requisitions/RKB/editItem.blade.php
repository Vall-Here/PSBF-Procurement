...
<div class="min-h-screen p-4 sm:ml-64">
    <div class="mt-14 rounded-lg p-4">
        <div class="container mx-auto px-4">
            ...
            <form action="{{ route('rkbs.items.update', $item->id) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="mb-4">
            ...
            <form action="{{ route('rkbs.items.delete', $item->id) }}" method="POST" style="display:inline-block;">
                @csrf
                @method('DELETE')
                <button type="button" onclick="return confirmDelete({{ $item->id }})"  class="bg-red-500 text-white px-2 py-1 rounded">Delete</button>
            </form>
        </div>
    </div>
</div>
